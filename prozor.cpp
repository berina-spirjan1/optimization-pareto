    ...
    : TForm(Owner)
{
	...
	if (ButtonPareto) {
        ButtonPareto->OnClick = ButtonParetoClick;
	}
}
//---------------------------------------------------------------------------
...
// Funkcija za handlanje iscrtavanja Pareto figure
void __fastcall TForm1::ButtonParetoClick(TObject *Sender) {
	vector<Tacka> paretoFigura = nadjiParetoFiguru(tacke);
	int canvasHeight = Slika->Height;

	// Iscrtavamo ponovno citav kanvas
	Slika->Canvas->Brush->Color = clWhite;
	Slika->Canvas->FillRect(TRect(0, 0, Slika->Width, Slika->Height));
	Slika->Canvas->Brush->Color = clBlack;
	Slika->Canvas->FrameRect(TRect(0, 0, Slika->Width, Slika->Height));

	// Iscravamo sve tacke i bojimo ih u plavo
	for (const auto& tacka : tacke) {
		tacka.Crtaj(Slika, clBlue, 2);
	}

	// Iscrtavamo tacke Pareto figure i bojimo ih u crveno
	for (const auto& tacka : paretoFigura) {
		tacka.Crtaj(Slika, clRed, 4);
	}

	// Iscrtavamo horizontalne i vertikalne linije Pareto figure
	Slika->Canvas->Pen->Color = clGreen;
	for (int i = 0; i < paretoFigura.size() - 1; ++i) {
		int xCurrent = paretoFigura[i].x;
		int yCurrent = paretoFigura[i].y;
		int xNext = paretoFigura[i + 1].x;
		int yNext = paretoFigura[i + 1].y;

		// Horizontalna linija
		Slika->Canvas->MoveTo(xCurrent, yCurrent);
		Slika->Canvas->LineTo(xNext, yCurrent);

		// Vertikalna linija
		Slika->Canvas->MoveTo(xNext, yCurrent);
		Slika->Canvas->LineTo(xNext, yNext);
	}

		// Iscrtavamo liniju od tacke sa najvecom x koordinatom koja pripada Pareto figuri i iscrtavamo liniju do x ose
		int firstX = paretoFigura.front().x;
		int firstY = paretoFigura.front().y;
		Slika->Canvas->MoveTo(firstX, firstY);
		Slika->Canvas->LineTo(firstX, canvasHeight);

		 //Iscrtavamo liniju od tacke sa najvecom y koordinatom koja pripada Pareto figuri i iscrtavamo liniju do y ose
		int lastX = paretoFigura.back().x;
		int lastY = paretoFigura.back().y;
		Slika->Canvas->MoveTo(lastX, lastY);
		Slika->Canvas->LineTo(0, lastY);

	ShowMessage("Pareto je iscrtan!");
}

//bool uporediTacke(const Tacka& a, const Tacka& b) {
//	if (a.x == b.x)
//		return a.y < b.y;
//	return a.x > b.x;
//}
...
//// Funkcija za handlanje iscrtavanja Pareto figure
//void __fastcall TForm1::ButtonParetoClick(TObject *Sender) {
//	// Odredjujemo tacke Pareto figure
//	vector<Tacka> paretoFigura = grahamPareto(tacke);
//	int canvasHeight = Slika->Height;
//
//	// Cistimo kanvas i ponovno sve iscrtavamo sto nam je potrebno
//	Slika->Canvas->Brush->Color = clWhite;
//	Slika->Canvas->FillRect(TRect(0, 0, Slika->Width, Slika->Height));
//	Slika->Canvas->Brush->Color = clBlack;
//	Slika->Canvas->FrameRect(TRect(0, 0, Slika->Width, Slika->Height));
//
//	// Iscrtavamo sve ostale tacke koje su postojale i bojimo ih u plavo
//	for (const auto& tacka : tacke) {
//		tacka.Crtaj(Slika, clBlue, 2);
//	}
//
//	// Provjeravamo da li postoje tacke Pareto figure prije nego sto ih iscrtamo
//	if (!paretoFigura.empty()) {
//		// Iscrtavamo tacke Pareto figure i bojimo ih crvenom bojom
//		for (const auto& tacka : paretoFigura) {
//			tacka.Crtaj(Slika, clRed, 4);
//		}
//
//
//		//Crtamo horizontalne i vertikalne linije za Pareto figuru
//		Slika->Canvas->Pen->Color = clGreen;
//		for (size_t i = 0; i < paretoFigura.size() - 1; ++i) {
//			int XTrenutno = paretoFigura[i].x;
//			int yTrenutno = paretoFigura[i].y;
//			int xSljedeca = paretoFigura[i + 1].x;
//			int ySljedeca = paretoFigura[i + 1].y;
//
//			// Iscrtavamo horizontalne linije
//			Slika->Canvas->MoveTo(XTrenutno, yTrenutno);
//			Slika->Canvas->LineTo(xSljedeca, yTrenutno);
//
//			// Crtamo vertikalne linije
//			Slika->Canvas->MoveTo(xSljedeca, yTrenutno);
//			Slika->Canvas->LineTo(xSljedeca, ySljedeca);
//		}
//
//		// Iscrtavamo liniju od tacke sa najvecom x koordinatom koja pripada Pareto figuri i iscrtavamo liniju do x ose
//		int firstX = paretoFigura.front().x;
